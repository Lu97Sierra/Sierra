...
import { IEditorOptions, ITextEditorOptions, IEditorPartOptions, IEditorInput, GroupDirection, GroupLocation, GroupOrientation, EditorGroupLayout, IEditorGroup, IMergeGroupOptions, IEditorLayout_Input, IAddGroupOptions, GroupsOrder, GroupsArrangement } from '../generated-model';
...
export interface IEditorGroupsService extends Pick<IVscodeEditorGroupsService, Exclude<keyof IVscodeEditorGroupsService, 'onDidActiveGroupChange' | 'onDidAddGroup' | 'onDidRemoveGroup' | 'onDidMoveGroup' | 'onDidActivateGroup' | 'onDidLayout' | 'orientation' | 'whenRestored' | 'getSize' | 'setSize' | 'arrangeGroups' | 'applyLayout' | 'centerLayout' | 'isLayoutCentered' | 'setGroupOrientation' | 'enforcePartOptions' | 'moveGroup' | 'mergeGroup' | 'copyGroup' | 'partOptions' | 'count'>> {
    setGroups(data: IEditorGroup[]): any;
    /**
     * Applies the provided layout by either moving existing groups or creating new groups.
     */
    applyLayout(layout: IEditorLayout_Input): void;
}
export declare const enum GroupChangeKind {
    GROUP_ACTIVE = 0,
    GROUP_LABEL = 1,
    EDITOR_OPEN = 2,
    EDITOR_CLOSE = 3,
